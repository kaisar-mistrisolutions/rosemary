    ...
    <nav class="flex" aria-label="Breadcrumb">
      <ol class="bg-white rounded-md shadow px-3 flex space-x-2 m mt-8 ml-8">
          ...
          <div class="flex items-center">
              ...
              <a href="{{ route('app.brands.index') }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700" aria-current="page">{{ $brand->name }}</a>
          </div>
          </li>
      </ol>
    </nav>
...
<div class="md:grid md:grid-cols-3 md:gap-6 pr-20 pl-10 pb-20 pt-10">
    <div class="md:col-span-1">
      <div class="px-4 sm:px-0">
        <h3 class="text-lg font-medium leading-6 text-gray-900">Update Brand</h3>
        <p class="mt-1 text-sm text-gray-600">
          Fill the form to update brand.
        </p>
      </div>
    </div>
    ...
    <div class="mt-5 md:mt-0 md:col-span-2">
      <form action="{{ route('app.brands.update',$brand->id)}}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
          ...
          <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
            <div class="grid grid-cols-3 gap-6">
              <div class="col-span-3 sm:col-span-2">
                ...
                <div class="mt-1 flex rounded-md shadow-sm">
                  <input type="text" name="name" value="{{ $brand->name }}" id="name" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-md sm:text-sm border-gray-300" placeholder="brand name">
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div x-data="{photoName: null, photoPreview: null}" class="col-span-2 ">
                ...
                <div class="mt-2" x-show="! photoPreview" value="{{ $brand->image }}" style="display: block;">
                    <img class="max-w-lg flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md" src="{{Storage::url($brand->image)}}" class="rounded-full h-20 w-20 object-cover">
                </div>
